...
makeLink activeIndex index component =
    let
        ...
        linkBuilder =
            \url -> li [] [ a [ href url, class classesAsString ] [ text title ] ]
    in
...
view config headerCollapsed active =
    let
        activeIndex =
            Maybe.withDefault (Random.minInt) active
